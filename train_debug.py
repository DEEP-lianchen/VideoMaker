...
class VideoMaker(torch.nn.Module):
    def __init__(self, pipe, unet):
        super().__init__()
        self.pipe = pipe
        self.unet = unet
    ...
    def samples(self, prompt, num_frames, guide_scale, reference_image_emb, negative_prompt, num_videos_per_prompt, num_inference_steps, device, seed=None):
        self.pipe.set_fusion_model(unet=self.unet)
        ...
        with torch.inference_mode:
            video = self.pipe(
                ...
                reference_image_emb=reference_image_emb,
...
def main():
    ...
    pipe = VideoMakerAnimateDiffPipeline(
        args.pretrained_base_model,
    ...
    videomaker = VideoMaker(pipe, unet)
    ...
    params_to_opt = [param for name, param in videomaker.unet.named_parameters() if param.requires_grad]
    ...
    videomaker, optimizer, train_dataloader = accelerator.prepare(videomaker, optimizer, train_dataloader)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if global_step % args.save_steps == 0 or global_step == 1:
                    ...
                    video_g = videomaker.module.generate(
                        device=next(videomaker.parameters()).device,
                        prompt=batch["video_prompt"],
                        reference_image_emb=ref_latents,
